...
from config import ActiveSpaceConfig
from results import ActiveSpaceResult


def run_asf(mol_pyscf: gto.Mole, cfg: ActiveSpaceConfig) -> ActiveSpaceResult:
    """Run ASF and return orbital → atom Mulliken mapping."""
    ...
    if cfg.method == "entropy":
        ...
        if cfg.size is None:
            raise ValueError("Set ActiveSpaceConfig.size when method='sized'")
        ...
        raise ValueError(f"Unknown active space method: {cfg.method!r}")
    ...
    # ── 2. Mulliken orbital → atom mapping ────────────────────────────────────
