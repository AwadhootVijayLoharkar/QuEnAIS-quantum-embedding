# ASF.py — test_8 — Active Space Finder
"""
Same five-phase logic as before (UHF + tier classification -> MP2
deviation proxy -> adaptive gap detection -> Loewdin population). Saves
the full spin-separated MP2 1-RDM (alpha, beta, and their sum, in AO
basis) into step1_asf.pkl so DMET.py's config.DMET_REFERENCE="mp2" path
never needs to recompute MP2 from scratch.

CHANGE vs test7: `import config` now happens FIRST, before numpy -- see
config.py's docstring for why (OpenBLAS/OpenMP thread-count env vars have
to be set before numpy's first import to actually take effect).

Usage: python ASF.py [--force]
"""
