...
"""
Consolidated DMET -> GQE-for-QSCI bridge (test6's dmet_molecule_adapter.py
+ dmet_excitation_pool.py merged, logic unchanged from test6).

FIX (this revision): `import DMET as dmet_step2` is gone. DMET.py is a
script -- importing it ran its argparse + cache-check code as a side
effect, and on a cache hit it called sys.exit(0) at module level, which
silently killed this whole process before any of this file's own code
ran (that's exactly what happened in your run: the only output was
DMET's "[Step 2] Using cached result" line, then nothing). Now imports
`dmet_lib` instead, which has zero module-level side effects.

Needs: pyscf, cudaq, openfermion, tequila (ClosedShellAmplitudes only),
and the external gqe_qsci package on the Python path.
"""
...
# NEW: import from dmet_lib (no side effects), NOT from DMET (a script).
import dmet_lib
...
# ═══════════════════════════════════════════════════════════════════════
# Part 2 — DMET-aware operator pools (unchanged from test6)
# ═══════════════════════════════════════════════════════════════════════
