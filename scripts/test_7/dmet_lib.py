...
THIS FILE EXISTS TO FIX A REAL BUG from your run: gqe_for_qsci.py did
...
on your cache hit, sys.exit(0) raised SystemExit and killed the entire
GQE_for_QSCI.py process before any of ITS OWN code ran. That's exactly
what the log showed: the only line printed was DMET's own
"[Step 2] Using cached result", then silent exit code 0 -- the CASCI /
consistency-check code in gqe_for_qsci.py's __main__ block never executed.

Fix: the reusable functions now live here, with zero side effects. Both
...
def get_reference_density(mf, mol, step1, mo_list, mo_coeff, method):
    ...
    NOTE vs the version embedded directly in DMET.py before: mo_list and
    mo_coeff are now explicit parameters instead of being read from
    DMET.py's module-level globals -- that implicit-global dependency is
    exactly the kind of thing that breaks silently when a function gets
    reused outside its original script's context, so it's fixed here too.
    """
    if method == "mp2":
...
def chemical_potential_correction(h1e_emb, n_emb, n_alpha, n_beta,
    ...
    """
    FIX vs the version that just ran on N2: mu_range="auto" (new default)
    derives the search bracket from h1e_emb's own eigenvalue spectrum
    instead of a fixed (-5, 5) Ha guess. Your N2 log showed exactly why
    the fixed guess breaks: 4 of the 8 embedding eigenvalues already sat
    below -5 Ha (shifted down by Phase D's core mean-field potential),
    while the target was 3 -- the true mu was outside the hardcoded
    range. The bisection correctly detected it couldn't bracket the
    target and safely skipped rather than silently returning a wrong
    answer -- that part worked as designed. This version fixes the root
    cause (a bad fixed guess) instead of just widening it further.
    """
